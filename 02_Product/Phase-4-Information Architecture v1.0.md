...
> **The Assessment Lifecycle ADR supersedes or extends this document** on: reviewer visibility (§17.2), vocabulary for "Marketplace" (§10), and the agent surface sitemap (§3, which predates the review queue). Each carries its own notice at the point of use.
> The **L1 Register** at §21.2 has been relocated to `06_Legal/L1 Register.md`, which is authoritative; §21.2 is retained as the historical record.
...
> **Consistency notice (Decisions E and G).**
> The **AGENT** block above omits **`/agent/reviews`** — the assessment review queue. That destination is defined in §6 and appears in the agent navigation in §12.4.
> The review queue is the BR-01 gate and, per §6.1, a deliberately separate destination from `/agent/matters`.
> The authoritative agent sitemap is **§6**; the authoritative agent navigation is **§12.4**. Navigation is not redesigned by this notice.
...
> **Superseded notice (Decisions C, D and G).**
> **"Marketplace" was overloaded in this section** and is corrected by ADR §12 and §13.8: the MVP agent-matching concept is **"Agent Matching / Engagement"**; **"Marketplace"** is reserved for the future licensing surface. They are distinct vocabulary entries.
> The **canonical terminology reference is now `01_Strategy/Glossary.md`** (Decision D), which carries structured entries for *Marketplace*, *Agent Matching*, *Review Grant* and the other domain terms, including their historical wording.
> The dual-register rule (§10.1), the client/agent labelling lexicon (§10.2), the banned-terms list (§10.3) and the action verbs (§10.4) below are **unchanged and remain in force** for product UI labelling. The Glossary does not replace them.
...
> **Extension notice — Reviewer visibility (Decision F).**
> The table above predates the Assessment Lifecycle ADR and contains no row for a Verified Agent holding a **review grant**.
> A reviewer's reach is strictly narrower than an assigned agent's: exactly one Disclosure version and its attachments, plus the analysis under review, for exactly one `ReviewDecision`, expiring on release or reassignment (ADR §6).
> A reviewer sees **nothing else in that Workspace** — not the client's other Inventions, portfolio, matters, documents or any aggregate view. Cross-tenancy invisibility (IA-5) applies unchanged: to a reviewer, those objects do not exist.
> This **expands** authorization without replacing Matter access. The Assigned Agent and Non-assigned Agent rows above are unchanged.
...
> **Relocation notice (Decision H1).**
> The L1 Register is now maintained as a living document at `06_Legal/L1 Register.md`, which is **authoritative**.
> The living register contains entries added after this document was frozen — including **L1-21** (platform-to-reviewer compensation for pre-engagement assessment review), which ADR §8 and §14 instructed be added to the register but which could not be landed here.
> The table below is **retained as the historical record** of the register as it stood at Phase 4's freeze. Where the two differ, the living document governs.
